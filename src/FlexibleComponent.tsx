...
export const FlexibleComponent = ({ className, style }: IFlexibleComponentProps) => {
  ...
  const _hideHeaderWidget = ctx.config.hideHeaderWidget || (isSingleComponent && ctx.config.hideHeaderWhenSingleComponent);

  const renderHeader = () => {
    // useFWHeaderSlot() 依赖这个 div
    const _slotEle = <div id={'FW-internal-slot-' + layout.key} className={cx('internal-slot')} />;
    const title = ctx.renderTitle(layout) || InnerComponent.displayText || layout.key;

    if (_hideHeaderWidget) {
      return (
        <div className={cx('header', 'no-extra')}>
          <div className='title'>
  ...
  const _extraProps = ctx.onComponentProps?.(layout);
  ...
  return (
    ...
    >
      ...
      <div id={`FW-component-body-${layout.key}`} className='body'>
        <ErrorBoundary key={layout.component}>{domReady && <InnerComponent {..._extraProps} />}</ErrorBoundary>
      </div>
    </div>
  );
};
